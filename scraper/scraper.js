...
async function scrapeGroup(group, groupIndex, totalGroups, targetPage, existingFacebookPostIds, existingTemporaryIds, allPostsData) {
    ...
    while (!stopGroup && scrollCount < MAX_SCROLL_COUNT && !isShuttingDown) {
        ...
        for (let i = processedIndex; i < count; i++) {
            ...
            // Completely skip Facebook feed sorting widgets (Most Relevant, Most recent, New activity, Suggested for you)
            ...
            // Method 4: Permalink fallback if still empty
            if (!finalBody && data.permalinkObj && data.permalinkObj.url && data.permalinkObj.url.startsWith("http")) {
                try {
                    const tempPage = await context.newPage();
                    await tempPage.goto(data.permalinkObj.url, { waitUntil: "domcontentloaded", timeout: 15000 });
                    await tempPage.waitForTimeout(2000);
                    let permalinkBody = await tempPage.evaluate(() => {
                        const messageEl = document.querySelector('div[data-ad-comet-preview="message"], div[data-ad-preview="message"]');
                        if (messageEl) {
                            return messageEl.innerText.replace(/See translation\n?/g, '').trim();
                        }
                        return "";
                    });
            ...
            if (!finalBody && imagesCount === 0 && videosCount === 0) {
                skipped = true;
                skipReason = "No body extracted and no media present.";
            }

            console.log("\n==============================");
            console.log("BODY EXTRACTION REPORT");
            console.log("==============================");
            console.log(`Method 1:\n${method1Success ? "SUCCESS" : "FAILED"}`);
            console.log(`Method 2:\n${method2Success ? "SUCCESS" : "FAILED"}`);
            console.log(`Method 3:\n${method3Success ? "SUCCESS" : "FAILED"}`);
            console.log(`Method 4:\n${method4Success ? "SUCCESS" : "FAILED"}`);
            console.log(`Final Source:\n${finalSource}`);
            console.log(`Characters:\n${finalBody ? finalBody.length : 0}`);
            console.log(`Images:\n${imagesCount}`);
            console.log(`Videos:\n${videosCount}`);
            console.log(`Skipped:\n${skipped ? "YES" : "NO"}`);
            if (skipped) console.log(`Reason:\n${skipReason}`);
            console.log("==============================\n");
